    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            try {
                ...
                const count = Object.keys(zip.files).filter((n) => !zip.files[n].dir && /\.(png|jpg|jpeg)$/i.test(n)).length;
                setImageCount(count);
            } catch {
                setImageCount(null);
            }
            ...
            console.error(err);
            setError(err.message || tConv("errorGeneric"));
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="max-w-5xl relative z-10">
                ...
                <div className="mb-12">
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div>
                            <h1 className="text-4xl font-display font-black text-[#0e0e0e] mb-3">{tConv("title")}</h1>
                        ...
                        <div className="glass rounded-3xl border border-slate-200 p-8 h-full flex flex-col">
                            {!resultUrl ? (
                                <>
                                    ...
                                    <div className="mb-6">
                                        <label className="block text-sm font-bold text-[#0e0e0e] mb-2">
                                            {tConv("pageRangeLabel")} <span className="font-normal text-[#888]">({tConv("optional")})</span>
                                        </label>
                                        <input
                                            type="text"
                                            value={pageRange}
                                            onChange={(e) => { setPageRange(e.target.value); setError(null); }}
                                            placeholder={tConv("pageRangePlaceholder")}
                                ...
                                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex-1 flex flex-col">
                                    <ConversionResultPanel
                                        filename="zygsoft_pdf_images.zip"
                                        fileSize={resultBlob.size}
                                        conversionType="PDF → Image"
                                        conversionTimeMs={conversionTimeMs ?? undefined}
                                        fileCount={imageCount ?? undefined}
                                        fileCountLabel={imageCount === 1 ? t("resultPanel.fileCountOne") : t("resultPanel.fileCount")}
                                        preview={<ImageThumbnailGrid zipBlob={resultBlob} />}
                                        downloadOptions={
                                            <a
                                                href={resultUrl}
                                                download="zygsoft_pdf_images.zip"
