...
export default function PdfCompressTool() {
    ...
    const tCmp = useTranslations("Dashboard.overview.tools.pdfCompress");
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            const res = await fetch("/api/tools/pdf-compress", {
                ...
                throw new Error(tCmp("errorService"));
            });

            if (!res.ok) {
                const errorData = await res.json().catch(() => ({}));
                throw new Error(errorData.error || tCmp("errorGeneric"));
            }
            ...
            console.error(err);
            setError(err instanceof Error ? err.message : tCmp("errorGeneric"));
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="relative">
                <div className="max-w-5xl relative z-10">
                    <Link
                        ...
                        className="inline-flex items-center gap-2 text-[#888] hover:text-[#0e0e0e] transition-colors mb-8 text-sm font-bold uppercase tracking-wider"
                    >
            ...
            <div className="max-w-5xl relative z-10">
                ...
                <div className="mb-12">
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div>
                            <h1 className="text-4xl font-display font-black text-[#0e0e0e] mb-3">{tCmp("title")}</h1>
                            <p className="text-[#666] font-medium text-lg max-w-2xl">{tCmp("description")}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-1">
                        ...
                        >
                            ...
                            <h3 className="text-xl font-display font-bold text-[#0e0e0e] mb-2">{tCmp("uploadTitle")}</h3>
                            <p className="text-[#888] text-sm font-medium leading-relaxed">{tCmp("uploadHint")}</p>
                            {file && (
                            ...
                            {!resultUrl ? (
                                <>
                                    {file && filePreviewUrl ? (
                                        <div className="mb-6 flex-1 flex flex-col min-h-0">
                                            ...
                                            <p className="mt-2 text-[11px] text-slate-400 font-medium">{tCmp("previewHint")}</p>
                                        </div>
                                    ) : (
                                        <div className="mb-6 flex-1 min-h-[200px] rounded-2xl border border-dashed border-slate-200 bg-slate-50/80 flex items-center justify-center">
                                            <p className="text-sm text-slate-400 font-medium text-center px-6">{tCmp("previewPlaceholder")}</p>
                                        </div>
                                    )}
                                ...
                                >
                                    <ConversionResultPanel
                                        filename="zygsoft_compressed.pdf"
                                        ...
                                        downloadOptions={
                                            <a
                                                href={resultUrl}
                                                download="zygsoft_compressed.pdf"
                                            ...
                                            >
                                                <Download size={16} /> {tCmp("downloadButton")}
                                            </a>
                                        }
                                        onReset={reset}
                                        successTitle={tCmp("successTitle")}
                                        successDesc={tCmp("successDesc")}
                                        newButtonLabel={tCmp("newButton")}
                                    />
                                </motion.div>
                            ) : null}
                        </div>
