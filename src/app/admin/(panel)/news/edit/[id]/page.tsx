...
function normalizePayload(data: BlogFormData, categoryId: string | null): Record<string, unknown> {
...
export default function AdminNewsEditPage() {
    ...
    useEffect(() => {
        fetch(`/api/blog/${id}`)
            ...
            .then((post) => {
                if (!post?.id || !isNewsCategory(post.category)) {
                    setInitialData(null);
                    return;
                }
                setNewsCategoryId(post.category_id || null);
                setInitialData({
                    slug: post.slug,
                    ...
                    tag_ids: post.tags?.map((tagItem: { tag_id?: string; tag?: { id: string } }) => tagItem.tag_id ?? tagItem.tag?.id) || [],
    ...
    const handleSubmit = async (data: BlogFormData) => {
        const payload = normalizePayload(data, newsCategoryId);
    ...
    if (!initialData) {
        return (
            <div className="text-center py-16">
                ...
                <Link href="/admin/news" className="mt-4 inline-block text-emerald-600 font-medium hover:underline">
                    Listeye dön
                </Link>
            </div>
        );
    ...
    return (
        <div className="space-y-8">
            <AdminPageHeader title="Haberi Düzenle" subtitle={initialData.title_tr} backHref="/admin/news" />
            <div className="rounded-2xl border border-slate-200/80 bg-white shadow-[var(--shadow-sm)] p-8">
                <BlogEditorForm
                    ...
                    hideCategoryField
                    fixedCategoryId={newsCategoryId ?? undefined}
                />
            </div>
        </div>
    );
