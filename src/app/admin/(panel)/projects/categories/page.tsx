...
export default function AdminProjectCategoriesPage() {
    ...
    const handleUpdate = async () => {
        ...
        setSubmitting(true);
        try {
            const res = await fetch(`/api/projects/categories/${editingId}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
    ...
    const handleDelete = async (id: string) => {
        ...
        if (count > 0) {
            alert(`${cat?.name_tr} kategorisi ${count} projede kullanılıyor. Önce projelerin kategorisini değiştirin.`);
            return;
        }
        ...
        setError(null);
        try {
            const res = await fetch(`/api/projects/categories/${id}`, { method: "DELETE" });
            const data = await res.json();
    ...
    return (
        <div>
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h1 className="text-3xl font-bold text-slate-900">Proje Kategorileri</h1>
                    <p className="text-slate-500 mt-1 text-sm">Proje kategorilerini yönetin.</p>
                </div>
            </div>
                                    ...
                                    {editingId === c.id ? (
                                        ...
                                        <div className="flex items-start justify-between gap-4">
                                            <div className="min-w-0">
                                                ...
                                                {c._count !== undefined && <span className="text-xs text-slate-400">{c._count.projects} proje</span>}
                                            </div>
                                            <div className="flex items-center gap-1 shrink-0">
                                                <button onClick={() => startEdit(c)} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg transition-colors" title="Düzenle">
                                                    <Edit size={16} />
                                                </button>
