...
function compileShader(gl: GL, type: number, source: string) {
  ...
  if (!gl.getShaderParameter(sh, gl.COMPILE_STATUS)) {
    console.error("[DarkVeil] Shader:", gl.getShaderInfoLog(sh));
    gl.deleteShader(sh);
    return null;
  }
  return sh;
...
function createGlProgram(gl: GL, vs: string, fs: string) {
  ...
  if (!gl.getProgramParameter(p, gl.LINK_STATUS)) {
    console.error("[DarkVeil] Program:", gl.getProgramInfoLog(p));
    gl.deleteProgram(p);
    return null;
  }
  return p;
...
/** Orijinal CPPN + hueShift (sarı remap / ekstra uniform yok) */
const fragment = `
#ifdef GL_ES
...
vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
  ...
  buf[0] = mat4(
    vec4(1.6794263,1.3817469,2.9625452,0.),
    vec4(-1.8834411,-1.4806935,-3.5924516,0.),
    vec4(-1.3279216,-1.0918057,-2.3124623,0.),
    vec4(0.2662234,0.23235129,0.44178495,0.)
  ) * buf[0] + mat4(
    vec4(-0.6299101,-0.5945583,-0.9125601,0.),
    vec4(0.17828953,0.18300213,0.18182953,0.),
    vec4(-2.96544,-2.5819945,-4.9001055,0.),
    vec4(1.4195864,1.1868085,2.5176322,0.)
  ) * buf[1] + mat4(
    vec4(-1.2584374,-1.0552157,-2.1688404,0.),
    vec4(-0.7200217,-0.52666044,-1.438251,0.),
    vec4(0.15345335,0.15196142,0.272854,0.),
    vec4(0.945728,0.8861938,1.2766753,0.)
  ) * buf[2] + mat4(
    vec4(-2.4218085,-1.968602,-4.35166,0.),
    vec4(-22.683098,-18.0544,-41.954372,0.),
    vec4(0.63792,0.5470648,1.1078634,0.),
    vec4(-1.5489894,-1.3075932,-2.6444845,0.)
  ) * buf[3] + mat4(
    vec4(-0.49252132,-0.39877754,-0.91366625,0.),
    vec4(0.95609266,0.7923952,1.640221,0.),
    vec4(0.30616966,0.15693925,0.8639857,0.),
    vec4(1.1825981,0.94504964,2.176963,0.)
  ) * buf[4] + mat4(
    vec4(0.35446745,0.3293795,0.59547555,0.),
    vec4(-0.58784515,-0.48177817,-1.0614829,0.),
    vec4(2.5271258,1.9991658,4.6846647,0.),
    vec4(0.13042648,0.08864098,0.30187556,0.)
  ) * buf[5] + mat4(
    vec4(-1.7718065,-1.4033192,-3.3355875,0.),
    vec4(3.1664357,2.638297,5.378702,0.),
    vec4(-3.1724713,-2.6107926,-5.549295,0.),
    vec4(-2.851368,-2.249092,-5.3013067,0.)
  ) * buf[6] + mat4(
    vec4(1.5203838,1.2212278,2.8404984,0.),
    vec4(1.5210563,1.2651345,2.683903,0.),
    vec4(2.9789467,2.4364579,5.2347264,0.),
    vec4(2.2270417,1.8825914,3.8028636,0.)
  ) * buf[7] + vec4(-1.5468478,-3.6171484,0.24762098,0.);
...
export default function DarkVeil({
  ...
  useLayoutEffect(() => {
    ...
    const gl =
      (canvas.getContext("webgl2", opts) as GL | null) ??
      (canvas.getContext("webgl", opts) as GL | null);

    if (!gl) {
      console.error("[DarkVeil] WebGL context yok");
      return;
    }
    ...
    const uScanLoc = gl.getUniformLocation(program, "uScan");
    ...
    const measureParent = () => {
      ...
      let cssW = Math.round(r.width) || parent.clientWidth;
      let cssH = Math.round(r.height) || parent.clientHeight;
      if (cssW < 1) cssW = window.innerWidth;
      if (cssH < 1) cssH = window.innerHeight;
      return { cssW, cssH };
    };

    const resize = () => {
      const { cssW, cssH } = measureParent();
      if (cssW < 1 || cssH < 1) return;
      const bufW = Math.max(1, Math.floor(cssW * dpr * resolutionScale));
      const bufH = Math.max(1, Math.floor(cssH * dpr * resolutionScale));
      canvas.width = bufW;
      canvas.height = bufH;
    ...
    const loop = () => {
      ...
      gl.uniform1f(uScanLoc, scanlineIntensity);
      gl.uniform1f(uScanFreqLoc, scanlineFrequency);
      gl.uniform1f(uWarpLoc, warpAmount);
  ...
  return (
    <canvas
      ref={ref}
      className={`darkveil-canvas ${className}`.trim()}
    />
  );
}
